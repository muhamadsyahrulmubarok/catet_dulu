  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex space-x-3">
        <select v-model="selectedMonth" @change="fetchData" class="rounded-lg border-gray-300 text-sm">
          <option v-for="month in months" :key="month.value" :value="month.value">
            {{ month.label }}
          </option>
        </select>
        <select v-model="selectedYear" @change="fetchData" class="rounded-lg border-gray-300 text-sm">
      ...
      <div class="stat-card">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-900">${{ totalVolume.toFixed(2) }}</p>
          </div>
      ...
      <div class="stat-card">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-gray-900">${{ avgTransaction.toFixed(2) }}</p>
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card">
        ...
        <div class="h-80">
          <DoughnutChart v-if="categoryChartData.labels.length > 0" 
      ...
      <div class="card">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">User Spending Distribution</h3>
        <div class="h-80">
          <BarChart v-if="userSpendingData.labels.length > 0" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="card">
        ...
        <div class="space-y-4">
          <div v-for="category in categoryAnalysis" :key="category.name" 
            ...
            <div class="text-right">
              <p class="font-bold text-gray-900">${{ category.total.toFixed(2) }}</p>
      ...
      <div class="card">
        ...
        <div class="space-y-4">
          <div class="p-3 bg-blue-50 rounded-lg">
            ...
            <p class="text-lg font-bold text-gray-900">{{ mostActiveUser.name }}</p>
            <p class="text-sm text-gray-500">{{ mostActiveUser.transactions }} transactions</p>
          </div>
          <div class="p-3 bg-green-50 rounded-lg">
            <p class="text-sm font-medium text-green-600">Highest Spender</p>
            <p class="text-lg font-bold text-gray-900">{{ highestSpender.name }}</p>
            <p class="text-sm text-gray-500">${{ highestSpender.amount.toFixed(2) }}</p>
          </div>
          <div class="p-3 bg-purple-50 rounded-lg">
            <p class="text-sm font-medium text-purple-600">Average per User</p>
            <p class="text-lg font-bold text-gray-900">${{ avgPerUser.toFixed(2) }}</p>
      ...
      <!-- Trends -->
      <div class="card">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Spending Trends</h3>
        <div class="space-y-4">
          <div class="p-3 bg-gray-50 rounded-lg">
            <p class="text-sm font-medium text-gray-600">Transaction Frequency</p>
            <p class="text-lg font-bold text-gray-900">{{ transactionFrequency.toFixed(1) }}</p>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Top Category</th>
            </tr>
          </thead>
            ...
            <tr v-for="user in detailedUserData" :key="user.telegram_id">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="ml-3">
                    <p class="text-sm font-medium text-gray-900">{{ user.first_name || 'Unknown' }}</p>
                    <p class="text-sm text-gray-500">@{{ user.username || 'no_username' }}</p>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ${{ parseFloat(user.total_amount || 0).toFixed(2) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ${{ user.expense_count > 0 ? (user.total_amount / user.expense_count).toFixed(2) : '0.00' }}
              </td>
...
import { DoughnutChart, BarChart } from 'vue-chartjs'
import {
  Chart as ChartJS,
  ArcElement,
  BarElement,
...
const totalVolume = computed(() => {
  return analytics.value.reduce((sum, item) => sum + parseFloat(item.total_amount || 0), 0)
})

const avgTransaction = computed(() => {
  const totalTransactions = analytics.value.reduce((sum, item) => sum + parseInt(item.expense_count || 0), 0)
...
const topCategory = computed(() => {
  ...
  analytics.value.forEach(item => {
    if (item.category) {
      categories[item.category] = (categories[item.category] || 0) + parseFloat(item.total_amount || 0)
    }
  })
...
const categoryChartData = computed(() => {
  ...
  analytics.value.forEach(item => {
    if (item.category && item.total_amount) {
      categories[item.category] = (categories[item.category] || 0) + parseFloat(item.total_amount)
    }
  })
...
const userSpendingData = computed(() => {
  ...
  analytics.value.forEach(item => {
    ...
    userTotals[key] = (userTotals[key] || 0) + parseFloat(item.total_amount || 0)
  })
...
const mostActiveUser = computed(() => {
  ...
  analytics.value.forEach(item => {
    ...
    userCounts[name] = (userCounts[name] || 0) + parseInt(item.expense_count || 0)
  })

  const top = Object.entries(userCounts).sort(([,a], [,b]) => b - a)[0]
  return top ? { name: top[0], transactions: top[1] } : { name: 'N/A', transactions: 0 }
})
...
const highestSpender = computed(() => {
  ...
  analytics.value.forEach(item => {
    const name = item.first_name || 'Unknown'
    userTotals[name] = (userTotals[name] || 0) + parseFloat(item.total_amount || 0)
  })
...
const detailedUserData = computed(() => {
  ...
  analytics.value.forEach(item => {
    ...
    if (item.category) {
      userMap[key].categories[item.category] = (userMap[key].categories[item.category] || 0) + parseFloat(item.total_amount || 0)
    }
  })

  return Object.values(userMap).map(user => ({
    ...user,
    category: Object.entries(user.categories).sort(([,a], [,b]) => b - a)[0]?.[0] || 'N/A'
...
async function fetchData() {
  loading.value = true
  try {
    const response = await $fetch(`/api/analytics?year=${selectedYear.value}&month=${selectedMonth.value}`)
...
function exportData() {
  const csvData = detailedUserData.value.map(user => ({
    ...
    'Total Spent': user.total_amount.toFixed(2),
    'Transaction Count': user.expense_count,
    'Average Transaction': user.expense_count > 0 ? (user.total_amount / user.expense_count).toFixed(2) : '0.00',
