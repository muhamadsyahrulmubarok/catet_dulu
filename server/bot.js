...
bot.on("text", async (ctx, next) => {
	...
	// Greeting detection
	const greetings = ["hai", "haii", "halo", "hello", "hi", "hey", "hallo"];
	if (greetings.some((greet) => text.startsWith(greet))) {
		ctx.reply(
			'👋 Hai! Selamat datang di Expense Tracker Bot!\n\nSaya siap membantu kamu mencatat pengeluaran.\n\nKetik pengeluaranmu, misal: "Kopi 15rb" atau kirim foto struk.\n\nButuh bantuan? Gunakan /help.'
		);
		return;
	}
	if (ctx.message.text.startsWith("/")) return next(); // Pass command to next handler
	...
	try {
		...
		const processedData = await geminiService.processText(userText);
		...
		const expense = await db.createExpense(
			...
			processedData.description || userText,
		...
		// Create confirmation message (no inline keyboard)
		const confirmationMessage = `
✅ *Pengeluaran Berhasil Ditambahkan! / Expense Added Successfully!*\n\n💰 Jumlah / Amount: Rp ${parseFloat(
			...
			processedData.date || new Date().toLocaleDateString()
		}
    `;
...
bot.on("photo", async (ctx) => {
	try {
		...
		const imageBuffer = await sharp(response.data).jpeg().toBuffer();

		// Process with Gemini
		const processedData = await geminiService.processImage(
			imageBuffer,
			"image/jpeg"
		);
		...
		if (!processedData.amount || processedData.amount <= 0) {
			await ctx.telegram.editMessageText(
				...
				`📸 Image processed but no clear expense amount found.\n\n📝 Extracted text: ${
					processedData.raw_text || "No text detected"
...
💰 Jumlah / Amount: Rp ${parseFloat(processedData.amount).toLocaleString(
			"id-ID"
		)}
...
📋 Teks mentah / Raw text: ${
			processedData.raw_text
				? processedData.raw_text.substring(0, 100) + "..."
				: "None"
		}
    `;
		...
		ctx.reply(
			"❌ Sorry, there was an error processing your image. Please try again or send the expense as text."
		);
	}
});
...
bot.command("report", async (ctx) => {
	try {
		...
		if (expenses.length === 0) {
			ctx.reply(
				"📊 Belum ada pengeluaran tercatat untuk bulan ini. Mulai tambahkan pengeluaran!\n\n📊 No expenses recorded for this month yet. Start adding some expenses!"
			);
			return;
		}

		let reportMessage = `📊 *Laporan Bulanan / Monthly Report - ${now.toLocaleDateString(
			"en-US",
			{ month: "long", year: "numeric" }
		)}*\n\n`;
		reportMessage += `💰 *Total Pengeluaran / Total Spent:* Rp ${parseFloat(
			total
		).toLocaleString("id-ID")}\n`;
		...
		monthlyReport.forEach((category) => {
			...
			reportMessage += `• ${category.category}: Rp ${parseFloat(
				category.total
			).toLocaleString("id-ID")} (${percentage}%)\n`;
		});
...
bot.command("expenses", async (ctx) => {
	try {
		...
		expenses.forEach((expense, index) => {
			...
			message += `${index + 1}. $${parseFloat(expense.amount).toFixed(2)} - ${
				expense.description
			}\n`;
...
// Add /recent as alias for /expenses
...
bot.command("total", async (ctx) => {
	try {
		...
		ctx.reply(
			`💰 *Total for ${monthName} ${year}:* $${parseFloat(total).toFixed(2)}`,
			{ parse_mode: "Markdown" }
		);
	} catch (error) {
...
bot.action("expenses", async (ctx) => {
	...
	ctx.deleteMessage();
	await bot.handleUpdate({
		message: { text: "/expenses", from: ctx.from, chat: ctx.chat },
	});
});
...
bot.action("total", async (ctx) => {
	...
	ctx.deleteMessage();
	await bot.handleUpdate({
		message: { text: "/total", from: ctx.from, chat: ctx.chat },
	});
});
	...
	try {
		...
		for (const user of users.rows) {
			try {
				...
				console.error(
					`Error sending report to user ${user.telegram_id}:`,
					error
				);
			}
		}
