...
const db = {
  ...
  // Expense operations
  async createExpense(telegramId, amount, description, category, date, imageUrl, rawText, processedText) {
    const query = `
      INSERT INTO expenses (telegram_id, amount, description, category, date, image_url, raw_text, processed_text)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING *;
    `;
    const result = await pool.query(query, [telegramId, amount, description, category, date, imageUrl, rawText, processedText]);
    return result.rows[0];
  },
